...
contract MyToken is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // You can mint NFTs upon deployment if you'd like. 
    // If so, just uncomment "ownerMint" by deleting the two slashes "//" and replace the number 10 with however many NFTs you would like to mint.
    ...
    // This functions allow the owner of this contract to set the Provenance hash. 
    // Read previous comment about provenance hash close to the top of this contract for more info
    ...
    function whitelistMint(uint256 amount, bytes32[] memory proof) public payable nonReentrant {
        ...
        require(
            amount <= whitelist_LIMIT - _alreadyMinted[sender],
            "Insufficient mints left"
        );
        require(msg.value == whitelist_PRICE * amount, "Incorrect payable amount");
    ...
    function publicMint(uint256 amount) public payable nonReentrant {
        ...
        require(
            amount <= public_LIMIT - _alreadyMinted[sender],
            "Insufficient mints left"
        );
        require(msg.value == public_PRICE * amount, "Incorrect payable amount");
